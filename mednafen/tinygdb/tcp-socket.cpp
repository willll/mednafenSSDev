...
auto TinySocket::getURL(u32 port, bool useIPv4) const -> std::string
{
...
auto TinySocket::open(u32 port, bool useIPv4) -> bool
{
    ...
    while(!stopServer) 
    {
      ...
      if(localSendBuffer.size() > 0) {
        ...
        if constexpr(TCP_LOG_MESSAGES) {
          printf("%.4f | TCP >: [%" PRIu64 "]: %.*s\n", (f64)chrono_millisecond() / 1000.0, localSendBuffer.size(), localSendBuffer.size() > 100 ? 100 : (int)localSendBuffer.size(), (char*)localSendBuffer.data());
        }
    ...
    while(!stopServer) 
    {
      ...
      if(length > 0) {
        ...
        if constexpr(TCP_LOG_MESSAGES) {
          printf("%.4f | TCP <: [%d]: %.*s ([%d]: %.*s)\n", (f64)chrono_millisecond() / 1000.0, length, length, (char*)receiveBuffer.data(), length, length, (char*)packet);
        }
      } else if(length == 0) {
        disconnectClient();
      } else {
...
auto TinySocket::close(bool notifyHandler) -> void
{
    stopServer = true;
...
auto TinySocket::disconnectClient() -> void
{
    wantKickClient = true;
}

auto TinySocket::sendData(const u8 *data, u32 size) -> void
{
    std::lock_guard guard{sendBufferMutex};

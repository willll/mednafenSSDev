...
OpenGL_Blitter *ogl_blitter = NULL;
...
med_imgui_init(window, glcontext);
 ...
 assert(dest_rect.h > 0);
   ...
   else // No special scaler:
   {
    if(ogl_blitter) {
     ...
     med_imgui_render_frame(eff_source_surface, &eff_src_rect, &dest_rect, &eff_src_rect, InterlaceField, evideoip, rotated);
    }
    else
    {
     SDL_to_MDFN_Surface_Wrapper m_surface(screen);
